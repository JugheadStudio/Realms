...
export default function ChatLayout({ params }) {
  ...
  return (
    <div className="flex flex-col h-100">
      <div className="flex-grow room-message-container">
        {messages.length === 0 ? (
          <p>No messages yet...</p>
        ) : (
          <div className="flex flex-col">
            {messages.map((message, index) => {
              ...
              return (
                <div key={index} className={`${message.role === 'user' ? 'player-message self-end' : 'bot-message self-start'}`}>
                  ...
                  {message.audioContent && message.role === 'system' && (
                    <button onClick={() => playAudio(message.audioContent)} className="audio-button">
                      Play Audio
                    </button>
                  )}
                </div>
      ...
      <form className="pt-4 flex" onSubmit={handleSendMessage}>
        <input
          className="flex-grow room-message-input"
          type="text"
          value={input}
          placeholder="Type your message..."
          onChange={(e) => setInput(e.target.value)}
        />
        <button type="submit" className="room-send-button">
          Send
        </button>
      </form>
    </div>
